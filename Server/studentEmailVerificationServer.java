...
public class studentEmailVerificationServer{
    ...
    public static void startServer() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(150)) {
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
            ...
            if (data != null) {
                ...
                if (fields.length == 1) {
                    ...
                    boolean sendVerification=sendVerificationCode(email);
                    if(sendVerification) {
                        out.write("VerificationCodeSent\n".getBytes());
                        }
                } else if (fields.length == 2) {
                    String email = fields[0];
                    String enteredCode = fields[1];

                    boolean verificationResult=verifyVerificationCode(email, enteredCode);
    ...
    public static Boolean sendVerificationCode(String email) {
        ...
        insertVerificationCodeIntoDatabase(email, verificationCode);
    ...
    public static void insertVerificationCodeIntoDatabase(String email, String verificationCode) {
        out.println(verificationCode);
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            if (connection != null) {
                String insertQuery = "UPDATE studentform SET verification_Code = ?, expiration_time = NOW() + INTERVAL 1 HOUR WHERE email = ?";
    ...
    private static void updateVerificationStatusInDatabase(String email, boolean verificationSuccess) {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            if (connection != null) {
                String updateVerificationStatusQuery = "UPDATE studentform SET is_verified = ? WHERE email = ?";
                try (PreparedStatement preparedStatement = connection.prepareStatement(updateVerificationStatusQuery)) {
                    preparedStatement.setBoolean(1, verificationSuccess);
                    preparedStatement.setString(2, email);
                    preparedStatement.executeUpdate();
                }
            }
    ...
    public static boolean verifyVerificationCode(String email, String enteredCode) {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            if (connection != null) {
                // Query the database to get the actual verification code
                String actualCode = getActualVerificationCodeFromDatabase(email);
                ...
                if (enteredCode.equals(actualCode)) {
                    ...
                    updateVerificationStatusInDatabase(email, true);
                    ...
                    updateVerificationStatusInDatabase(email, false);
    ...
    public static String getActualVerificationCodeFromDatabase(String email) {
        String verificationCode = null;

        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement preparedStatement = connection.prepareStatement("SELECT verification_Code FROM studentform WHERE email = ?")) {
